...
(function() {
  ...
  function main() {
    ...
    window.AXLIB = window.axlib = {
      ...
      // Load other modules
      layout,
      // Preset interface element display, hiding and scaling
      trace,
      // Display a text with a preset style
      jsonbin: getJsonBinClass(),
      // Call jsonbin v3 method to read and write data online
      hosting,
      // The parameter defines where to sync to, the default is axhub
      loading,
      //Insert a loading animation
      find,
      // Conveniently extract the object of the specified component name, example: let els = axlib.find('sym1', 'sym2'...); let { sym1, sym2 } = els;
      saveTextFile,
      // Pass in a string and a file name to save the file
      testExp,
      // Wait for the method to test the expression
      loadJsModule,
      // Load js as es module
      insertJsModule,
      // Insert js in string form as es module
      insertJS,
      //Insert JS into body as string
      insertCSS,
      // Insert CSS into head as string
      insertHTML,
      // Insert HTML into the specified node as a string
      number,
      // Get the element by component name
      loadOnce,
      // Load the specified resource only once, and use the lock parameter to prevent other scripts from loading the resource repeatedly, and then use the check parameter to allow other scripts to access the resource when it is ready
      plugins: getPlugins(),
