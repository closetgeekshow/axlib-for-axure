...
(function(){
  ...
  function main() {
    if (!window.$axure || !window.jQuery) {
      console.error('Please load $axure and jquery first!');
      return;
    }
    ...
    window.AXLIB = window.axlib = {
      ...
      use, // Load other modules
      layout, // Preset interface element display, hiding and scaling
      trace, // Display a text in a preset style
      jsonbin: getJsonBinClass(), // Call jsonbin v3 method to read and write data online
      hosting, // parameter defines where to sync to, default is axhub
      loading, // Insert a loading animation
      find, // Conveniently extract the object of the specified component name, example: let els = axlib.find('sym1', 'sym2'...); let { sym1, sym2 } = els;
      saveTextFile, // Pass in a string and file name to save the file
      testExp, // Waiting for the test expression method
      loadJsModule, // load js as es module
      insertJsModule, // insert js in string form as es module
      insertJS, // insert JS into body as string
      insertCSS, // insert CSS into head as a string
      insertHTML, // Insert HTML into the specified node as a string
      getEl, // Get the element by component name
      loadOnce, // Load the specified resource only once, and use the lock parameter to prevent other scripts from loading the resource repeatedly, and then use the check parameter to allow other scripts to access the resource when it is ready
  ...
  function InitAxlib() {
    ...
    // Rewrite and expand Axure's private objects
    $axure.internal(function ($ax) {

      /**
       * Enhanced repeater related methods
       * Note: Only single operations are supported. To ensure that each element has a different ID, you can use a loop to modify them one by one.
       */

      // Refresh data (to be defined first, because it will be referenced later)
      ...
      // It seems to get the row number, starting from 1, because it will be used later, so keep it
      ...
      $axure.ex.formatRepeaterData = function(data, options) {
        //Format the data according to the options parameter
        ...
        // Automatic mode, returns data in the form of "key: value" pairs
        if (format === 'default') {
          newData = data;
        } else if (format === 'auto') {
          let arr = [];
        ...
        // Row mode, horizontally obtain data, support trim to obtain the first bit of data
        else if (format === 'row') {
          let result = {};
          for (let i = 0; i < data.length; i++) {
            let item = data[i];
        ...
        // Column mode, get data vertically
        ...
        //Key mode, get data with a key value
        else if (format === 'key') {
          if (!options.key) {
            console.log('No Key name is provided, no formatting will be performed...');
            newData = data;
          } else {
            let result = {};
        ...
        // Tree mode, suitable for tree menus, supports trim to clean up empty data
        else if (format === 'tree') {
          let arr = [];
          for (let i = 0; i < data.length; i++) {
            let item = data[i];
          ...
          trace(`Unregistered format parameter: ${format}, please use this definition { format: 'auto' || 'row' || 'column' || 'tree' || 'key' } , where key mode needs to additionally provide the index key column name.`);
          newData = data;
        }
        return newData;
      }
      ...
      // Get the difference data after initialization and interactive appending
      ...
      // This will trigger the addition of data for native interaction
      ...
      // This will trigger the deletion of native interaction data
      ...
      /**
       * Enhanced dynamic panel related methods
       * Note: Only single operations are supported. To ensure that each element has a different ID, you can use a loop to modify them one by one.
       */
      ...
      /**
       * Simplify the method name for obtaining ID and node
       */
      ...
      /**
       * Fixed the bug that Axure cannot get global variables
       */
      ...
      /**
       * Add some common style controls
       */
      ...
      /**
       * Jump to page or link
       */
      
      // Set the Hook before the page jumps
      // Note: There is no hook when the page is loaded. If you want to do the transition animation before loading, you need to handle it yourself, because the time when axlib starts executing is always slower than when the page is rendered.
      ...
      // Cache the original jump method first
      $axure.ex.navigate = $ax.public.navigate;

      // Rewrite the built-in jump function
  ...
  function getDiffData(data1, data2) {
    // Deprecated, will accidentally delete the same data
  ...
  function testExp() {
    console.error('axlib.testExp() is deprecated');
  }
  ...
  function hosting(cloud) {
    ...
    if (cloud == 'axhub') {
      ...
      if (btnUpload.length === 0) {
        trace('Unable to sync, you need to install the axhub plugin for your browser first!');
      } else {
        btnUpload.click();
      }
    }
  ...
  function use() {
    console.error('axlib.use() is deprecated');
  }
  ...
  function utils() {
    // Add a method to remove line breaks from a string
    ...
    // Add a method to determine whether the variable conforms to the JSON format
    $.isJSON = function (str) {
      try {
    ...
    // Add a method to determine whether a DOM element has an Attribute attribute
    $.fn.extend({
      'hasAttr': name => {
